...
class DatasetConverter:
    ...
    def get_encoder(self, df, col_name):
        df = df.withColumn(
            col_name, F.coalesce(F.col(col_name).cast("string"), F.lit("#EMPTY"))
        )
    ...
    def log_transform(self, df, col_name):
        ...
        df = df.withColumn(
            col_name,
            F.signum(F.col(col_name)) * F.log(F.abs(F.col(col_name)) + F.lit(1)),
        )
        return df
    ...
    def _td_datetime(self, df, col_event_time):
        df = df.withColumn(
            "event_time", F.unix_timestamp(F.col(col_event_time)) / F.lit(24 * 60 * 60)
        )
    ...
    def _td_gender(self, df, col_event_time):
        ...
        'd hh:mm:ss' -> float where integer part is day
        number and fractional part is seconds from day begin
        '1 00:00:00' -> 1.0
        '1 12:00:00' -> 1.5
        '1 01:00:00' -> 1 + 1 / 24
        '2 23:59:59' -> 1.99
    ...
    def remove_long_trx(self, df, max_trx_count, col_client_id):
        df = df.withColumn(
            "_cn", F.count(F.lit(1)).over(Window.partitionBy(col_client_id))
        )
        df = df.withColumn(
            "_rn",
            F.row_number().over(
    ...
    def collect_lists(self, df, col_id):
        ...
        df = df.withColumn(
            ...
            "_rn", F.row_number().over(Window.partitionBy(col_id).orderBy("event_time"))
        )
    ...
    def split_dataset(self, all_data, test_size, df_target, col_client_id, salt):
        ...
        s_clients = set(
            cl[0] for cl in df_target.select(col_client_id).distinct().collect()
        )

        # shuffle client list
        s_all_data_clients = set(
            cl[0] for cl in all_data.select(col_client_id).distinct().collect()
        )
        ...
        s_clients_test = spark.createDataFrame(
            [(i,) for i in s_clients_test], [col_client_id]
        ).repartition(1)
        s_clients = spark.createDataFrame(
            [(i,) for i in s_clients], [col_client_id]
        ).repartition(1)
    ...
    def split_dataset_predefined(
        ...
        all_data = all_data.withColumn(
            "_is_test", F.coalesce(F.col("_is_test"), F.lit(0))
        )
    ...
    def load_transactions(self):
        ...
        if len(self.config.dict) > 0:
            ...
            for i in range(len(self.config.dict) // 2):
                ...
                spark.sparkContext.setLocalProperty(
                    "callSite.short", f"join with {i}th dict"
                )
                source_data = self.join_dict(
                    source_data, self.config.dict[2 * i], self.config.dict[2 * i + 1]
                )
        ...
        if cols_event_time[0][0] == "#":
            if cols_event_time[0] == "#float":
                ...
                raise NotImplementedError(
                    f'Unknown type of data transformation: "{cols_event_time[0]}"'
                )
        else:
