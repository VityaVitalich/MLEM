...
def trx_to_features(
    ...
    def _td_gender(df, col_event_time):
        ...
        'd hh:mm:ss' -> float where integer part
        is day number and fractional part is seconds from day begin
        '1 00:00:00' -> 1.0
        '1 12:00:00' -> 1.5
        '1 01:00:00' -> 1 + 1 / 24
        '2 23:59:59' -> 1.99
    ...
    features = (
        df_data[used_columns]
        .assign(et_index=lambda x: x["event_time"])
        .set_index([col_client_id, "et_index"])
        .sort_index()
        .groupby(col_client_id)
        .apply(lambda x: {k: np.array(v) for k, v in x.to_dict(orient="list").items()})
        .rename("feature_arrays")
        .reset_index()
...
def update_with_target(features, data_path, target_files, col_client_id, col_target):
    df_target = pd.concat(
        [pd.read_csv(os.path.join(data_path, file)) for file in target_files]
    )
    ...
    features = [
        dict(
            [("target", d_clients.get(rec[col_client_id], {}).get(col_target))]
            + list(rec.items())
        )
        for rec in features
    ]
...
if __name__ == "__main__":
    ...
    if config.log_file is not None:
        ...
        handlers = None
    logging.basicConfig(
        level=logging.INFO, format="%(funcName)-20s   : %(message)s", handlers=handlers
    )
    ...
    if config.test_size > 0:
        ...
        test_ids = pd.DataFrame(
            {config.col_client_id: [rec[config.col_client_id] for rec in test]}
        )
