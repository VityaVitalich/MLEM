...
class Pipeline():
    ...
    def run_experiment(self, run_name=None, conf=None, model_conf=None, seed=0):
        ...
        logger, ch, fh = self.setup_logging(run_name, self.log_dir, self.console_log, self.file_log)
        ### Fix randomness ###
        seed_everything(
    ...
    def optuna_setup(self, target_metric="val_metric", request_list=[], n_startup_trials=3, n_trials=50, n_runs=3):
        """
        ...
        try:
            first = False
            study = optuna.load_study(study_name=f"{self.log_dir / self.run_name}", storage="sqlite:///example.db")
        except:
            first = True
    ...
    def _objective(
        ...
        trial.set_user_attr("memory_before", torch.cuda.max_memory_reserved(self.device))
        ...
        return summary_df.loc["mean", target_metric] - summary_df.loc["std", target_metric]
