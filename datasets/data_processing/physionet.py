...
class PhysioNet(object):
    ...
    def create_parquet(self):
        ...
        self.cfg["data_path"] = os.path.join(self.processed_folder, "data.csv")
        dataset = SequenceDataset(self.cfg)
    ...
    def create_csv(self):
        ...
        print("Creating csv...")
        train = torch.load(
            os.path.join(self.processed_folder, self.training_file), map_location="cpu"
        )
        test = torch.load(
            os.path.join(self.processed_folder, self.test_file), map_location="cpu"
        )

        df = pd.DataFrame(columns=["user"] + self.params + ["event_time", "target"])
        for data in [train, test]:
            for user_idx, (user, time, vals, mask, label) in enumerate(tqdm(data)):
                df_mini = pd.DataFrame(columns=df.columns)
                vals[mask == 0] = -1
                for i, col in enumerate(self.params):
                    df_mini[col] = np.array(vals[:, i])
                df_mini["user"] = user
                df_mini["event_time"] = np.array(time) / 48
                df_mini["target"] = label.item() if label is not None else None
                df = pd.concat((df, df_mini))
        df.to_csv(os.path.join(self.processed_folder, "data.csv"), index=False)

    def _check_csv(self):
        if os.path.exists(os.path.join(self.processed_folder, "data.csv")):
            print("Csv already exists.")
            return True
        return False

    def _check_parquet(self):
        if os.path.exists(os.path.join(self.processed_folder, "train.parquet")):
        ...
        for url in self.outcome_urls:
            ...
            with open(txtfile) as f:
                ...
                for line in lines[1:]:
                    ...
                    record_id, labels = line[0], np.array(line[1:]).astype(float)
        ...
        for url in self.urls:
            ...
            for txtfile in tqdm(os.listdir(dirname)):
                ...
                if record_id in outcomes:
                    ...
                    # Out of 5 label types provided for Physionet,
                    # take only the last one -- mortality
                    labels = labels[4]
