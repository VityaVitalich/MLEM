...
class SplitByWeeks(AbsSplit):
    ...
    def split(self, dates):
        seq_len = len(dates)
        n_weeks = (
            (dates - dates[0]).astype("timedelta64[D]") // self.week_length
        ).astype(np.int16)
        ...
        split_indexes = np.random.randint(
            0, self.split_count, n_weeks_nunique - self.split_count
        )
        week_range = np.arange(self.split_count, n_weeks_nunique)
        n_weeks_idxes = [
            week_range[split_indexes == i] for i in range(self.split_count)
        ]
        ...
        x = n_weeks.reshape(1, -1).repeat(
            n_weeks_nunique, axis=0
        ) == n_weeks_unique.reshape(-1, 1).repeat(seq_len, axis=1)
        n_byweeks_idxes = [
            x[one_week_idxes].sum(axis=0).nonzero(as_tuple=False)[0]
            for one_week_idxes in n_weeks_idxes
        ]
        n_byweeks_idxes = [
            x[-self.cnt_max :] if len(x) > self.cnt_max else x for x in n_byweeks_idxes
        ]
...
class SampleSlices(AbsSplit):
    def __init__(
        self, split_count, cnt_min, cnt_max, short_seq_crop_rate=1.0, is_sorted=False
    ):
        self.split_count = split_count
        self.cnt_min = cnt_min
        self.cnt_max = cnt_max
    ...
    def split(self, dates):
        ...
        if (
            int(date_len * self.short_seq_crop_rate) <= self.cnt_min
        ...
        start_pos = (
            np.random.rand(self.split_count) * (available_start_pos + 1 - 1e-9)
        ).astype(int)
        if not self.is_sorted:
            return [date_range[s : s + leng] for s, leng in zip(start_pos, lengths)]

        ix_sort = np.argsort(start_pos)
        return [
            date_range[s : s + leng]
            for s, leng in zip(start_pos[ix_sort], lengths[ix_sort])
        ]
...
class SampleUniform(AbsSplit):
    ...
    def split(self, dates):
        ...
        start_pos = (
            np.linspace(0, date_len - self.seq_len, self.split_count)
            .round()
            .astype(int)
        )
...
def create(split_strategy, **params):
    ...
    if not issubclass(cls, AbsSplit):
        raise AttributeError(
            f'Wrong split_strategy: "{split_strategy}". This is not a splitter'
        )
    if cls is AbsSplit:
        raise AttributeError(
            f'Wrong split_strategy: "{split_strategy}". Splitter can not be abstract'
        )
