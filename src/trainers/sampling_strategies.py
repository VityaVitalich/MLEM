...
class DistanceWeightedPairSelector(PairSelector):
    ...
    def get_pairs(self, x, labels):
        ...
        log_weights = (2.0 - float(d)) * distance.log() - (
            float(d - 3) / 2
        ) * torch.log(torch.clamp(1.0 - 0.25 * (distance * distance), min=1e-8))
        ...
        weights = (
            weights * mask * ((distance < self.nonzero_loss_cutoff).float()) + 1e-8
        )
        ...
        return torch.LongTensor(positive_pairs).to(self.device), torch.LongTensor(
            negative_pairs
        ).to(self.device)
...
class HardTripletSelector(TripletSelector):
    ...
    def get_triplets(self, embeddings, labels):
        ...
        mat_distances = outer_pairwise_distance(
            acnhor_embed, embeddings.detach()
        )  # pairwise_distance anchors x all
...
class SemiHardTripletSelector(TripletSelector):
    ...
    def get_triplets(self, embeddings, labels):
        ...
        neg_mat_distances = mat_distances * (x == 0).type(
            mat_distances.dtype
        )  # filter: get only negative pairs
        ...
        negatives_outside = (upper_bound - neg_mat_distances) * (
            neg_mat_distances > D_ap.expand(n, m).t()
        ).type(neg_mat_distances.dtype)
        ...
        semihard_exist = (
            (neg_mat_distances > D_ap.expand(n, m).t()).sum(dim=1) > 0
        ).view(-1, 1)

        negatives_indeces = torch.where(
            semihard_exist, negatives_outside, negatives_inside
        )
...
def get_sampling_strategy(params):
    if params.loss.sampling_strategy is None:
        sampling_strategy = None

    elif params.loss.sampling_strategy == "AllPositivePair":
        sampling_strategy = AllPositivePairSelector()

    elif params.loss.sampling_strategy == "HardNegativePair":
        kwargs = {
            "neg_count": params.get("loss.neg_count", None),
        }
        kwargs = {k: v for k, v in kwargs.items() if v is not None}
        sampling_strategy = HardNegativePairSelector(**kwargs)

    elif params.loss.sampling_strategy == "AllTriplets":
        sampling_strategy = AllTripletSelector()

    elif params.loss.sampling_strategy == "RandomNegativeTriplets":
        sampling_strategy = RandomNegativeTripletSelector()

    elif params.loss.sampling_strategy == "HardTriplets":
        kwargs = {
    ...
    elif params.loss.sampling_strategy == "SemiHardTriplets":
        sampling_strategy = SemiHardTripletSelector()

    elif params.loss.sampling_strategy == "DistanceWeightedPair":
        kwargs = {
        ...
        raise AttributeError(
            f'wrong sampling_strategy "{params["train.sampling_strategy"]}"'
        )
